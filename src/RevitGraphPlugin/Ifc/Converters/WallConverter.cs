...
public sealed class WallConverter : IElementConverter
{
    ...
    public void Convert(Element element, IfcModelContext ctx)
    {
        ...
        var ox = UnitUtils.ConvertFromInternalUnits(origin.X, UnitTypeId.Millimeters);
        var oy = UnitUtils.ConvertFromInternalUnits(origin.Y, UnitTypeId.Millimeters);
        var placement = new IfcLocalPlacement(
            storey.ObjectPlacement,
            new IfcAxis2Placement3D(new IfcCartesianPoint(db, ox, oy, 0.0)));
